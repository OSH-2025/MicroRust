...
# ==================== 配置 ====================
IPFS_API_URL = "http://192.168.152.128:5001"
...
def parse_command(message: str, cid: str) -> List[List[str]]:
    ...
    try:
        ...
        prompt = (
            ...
            f"5. 每条指令仅返回一条结构\n\n"
            f"6. 添加句子的两条指令关键词为添加的内容，删除句子的关键词为空列表，查找的关键词为查找内容，下载的关键词为下载路径\n"
        ...
        try:
            ...
            # 尝试解析为列表
            parse_result = eval(cleaned_result)
            print(f"parse_result 的值: {parse_result}")
            print(f"parse_result 的类型: {type(parse_result)}")
            
            # 验证结果格式
            if (isinstance(parse_result, list) and len(parse_result) == 4 and
                all(isinstance(item, list) for item in parse_result)):
                return parse_result
            else:
...
def process_message(message: str, chat_history: list, confirmed: bool = False) -> Tuple[str, list, str]:
    """处理用户消息并执行相应操作"""
    global pending_operation, waiting_for_confirmation
    ...
    try:
        ...
        if message.strip().lower() == "cancel":
            if waiting_for_confirmation:
                waiting_for_confirmation = False
                pending_operation = None
                return "", chat_history, "❌ 操作已取消"
            else:
                return "", chat_history, "⚠️ 没有待取消的操作"
        
        # 如果是确认操作
        if confirmed:
            if not waiting_for_confirmation:
                return "", chat_history, "⚠️ 没有待确认的操作"
            
            # 执行实际操作的代码
            operation, keywords, filetype, real_cid = pending_operation
            ...
            chat_history.append((f"确认执行: {message}", result_text))
            waiting_for_confirmation = False
            pending_operation = None
        ...
        filetype = file_type[0] if file_type else "unknown"
        real_cid = cid_list[0] if cid_list else cid
        
        filename = uploaded_files.get(real_cid, ("未知文件",))[0]
        
        # 保存待确认的操作
        pending_operation = (operation, keyword_list, filetype, real_cid)
        ...
        confirm_msg = (
            ...
            f"📄 文件类型: {filetype}\n"
            f"-------------------------\n"
            f"请点击"确认操作"或"取消操作"按钮"
        )
        
        # 添加确认消息到聊天历史
        chat_history.append((message, confirm_msg))
        return "", chat_history, "🟡 请确认操作"
...
with gr.Blocks(title="IPFS文件操作系统") as app:
    ...
    with gr.Row():
        ...
        clear_btn = gr.ClearButton([msg, chat_history, upload_status])
    ...
    submit_btn.click(
        ...
        outputs=[msg, chat_history, status_display]
    )
    confirm_btn.click(
    ...
    cancel_btn.click(
        lambda x, y: process_message("cancel", y), 
        inputs=[msg, chat_history], 
        outputs=[msg, chat_history, status_display]
    )
    msg.change(
