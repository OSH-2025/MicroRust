...
在选用大模型方面，我们仍然进行了本地部署与远程api使用的部署测试与对比，我们在对比了效率与成本以后，仍然选择了api远程获得相应信息，我们选择了不同的大模型进行了不同的尝试，例如：deepseek,星火,AzureOpenAI,同时我们的项目支持大模型的修改，我们的模型也支持多模态的使用。
...
相对于中期汇报进行的优化，我们在使用gradio前端实现了用户命令解析版本后，发现由于操作要求的精准性，频繁地返回预览与操作极易导致用户操作的失误对于文件系统造成的毁灭性影响，我们最终选择放弃用户命令解析，而是让用户在基于搜索上进行精准的操作。
...
## 最终项目与中期设计对比
实现了中期回报的的构想，根据实际的应用，选择对于一些细节进行了具体的优化，从而大大提高了项目的可使用性。
...
本项目在原有系统架构基础上进行了全面升级与整合，成功实现了基于IPFS分布式存储系统的文件管理解决方案。通过整合图文件系统（Graph-based File System）与IPFS分布式协议作为核心后端，我们构建了一个高可扩展、去中心化的数据存储网络，有效解决了传统集中式存储的性能瓶颈与单点故障问题。
